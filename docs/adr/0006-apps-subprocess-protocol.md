...
8. **`protocol/` owns the control frames and the metrics shape.** The JSON-RPC `ping` method is
    ...
    object, and folding them into JSON-RPC would buy nothing. The `metrics` notification payload,
    `{ pid }`, gets its type in the same place, so neither half restates the shape.
